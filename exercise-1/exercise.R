...
ggplot(diamonds.sample) +
  geom_point(mapping = aes(carat, cut, size = price))

# Try coloring the above plot based on the diamond's price!
ggplot(diamonds.sample) +
  geom_point(mapping = aes(carat, cut, size = price, color = price))
...
ggplot(diamonds.sample) + 
  geom_line(mapping = aes(carat, price, color = carat))

# That's kind of messy. Try using `smooth` geometry instead.
ggplot(diamonds.sample) + 
  geom_point(mapping = aes(carat, price)) + 
  geom_smooth(mapping = aes(carat, price, color = carat))
...
ggplot(diamonds.sample) + 
  geom_histogram(mapping = aes(price, fill = clarity))

# (For a more traditional "bell-curve", make a histogram of diamond `depths`)
ggplot(diamonds.sample) + 
  geom_histogram(mapping = aes(depth))
...
ggplot(data = diamonds.sample) +
  geom_point(mapping = aes(carat, price, color = cut), alpha = 0.1) +
  geom_smooth(mapping = aes(carat, price, color = cut), se = FALSE)
